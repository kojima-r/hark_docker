...
RUN apt-get update -y && \
    apt-get upgrade -y && \
    apt-get install -y wget bzip2 curl git lsb-release
RUN mkdir -p ~/usr/hark
WORKDIR /usr/hark

RUN apt-get install -y gnupg sudo
...
RUN cat /etc/apt/sources.list.d/hark.list
RUN curl -sL https://deb.nodesource.com/setup_10.x | bash -
RUN apt-get update
RUN apt-get install -y nodejs hark-base harkmw hark-core hark-designer harktool5 harktool5-gui kaldidecoder-hark python3-pip nginx
